...
export default class HuChecker {
    ...
    // private checkCards( result : Group, cards : number[] , mustKeZi : boolean, vBaiDa : number[] , isQuick : boolean , ischeckHu : boolean ) : boolean  
    // {
    //     if ( cards.length == 0 )
    //     {
    //         // if ( vBaiDa != null && vBaiDa.length > 0 )
    //         // {
    //         //     if ( result.isHaveNotShun() || result.isHaveSingleCard() || result.getPairCnt() >= 2 )
    //         //     {
    //         //         return false ;
    //         //     }
    ...
    //         //     {
    //         //         let g = new Group();
    //         //         g.cards.push( vBaiDa[0],vBaiDa[1],vBaiDa[2] );
    //         //         result.addChild(g);
    //         //         vBaiDa.splice(0,3) ;
    //         //         return this.checkCards(g,cards,mustKeZi,vBaiDa ) ;
    //         //     }
    //         //     else
    //         //     {
    //         //         let g = new Group();
    ...
    //         this.removeValueFromArray(vSub,curValue);
    //         let ok = this.checkCards(g,vSub,mustKeZi,vBaiDa,isQuick,ischeckHu );
    //         if ( ok == false )
    //         {
    //             result.popChild();
    //         }
    ...
    //     // when lack of only one to Ke  , or be jiang 
    //     if (  keIdx0 != -1 )
    //     {
    //         if ( result.isHaveSingleCard() == false && ( result.getPairCnt() == 0 || ( ischeckHu == false && result.getPairCnt() == 1 && result.isHaveNotShun() == false && isKeOk == false ) ) )
    //         {
    //             let g = new Group();
    //             g.cards.push( curValue,curValue );
    //             result.addChild(g);
    ...
    //         // use bai da to ke 
    //         if ( false == isKeOk && vBaiDa != null && vBaiDa.length >= 1 )
    //         {
    //             let baiDa = vBaiDa[0] ;
    //             let g = new Group();
    ...
    //     if ( mustKeZi == false && ( ( shunIdx0 == -1 && shunIdx1 != -1 ) || ( shunIdx0 != -1 && shunIdx1 == -1 ) ) )
    //     {
    //         let usedValue = 0 ;
    //         if ( shunIdx0 == -1 && shunIdx1 != -1 )
    //         {
    //             usedValue = cards[shunIdx1] ;
    //         }
    //         else if ( shunIdx0 != -1 && shunIdx1 == -1 )
    //         {
    //             usedValue = cards[shunIdx0] ;
    //         }

    //         if ( ischeckHu == false && (result.getPairCnt() <= 1 && result.isHaveNotShun() == false) && result.isHaveSingleCard() == false )
    //         {
    //             let g = new Group();
    //             g.cards.push( curValue,usedValue );
    //             result.addChild(g);
    ...
    //     // only this card 
    //     if ( ischeckHu == false && result.isHaveSingleCard() == false && result.getPairCnt() == 0 && result.isHaveNotShun() == false ) // try to be single jiang 
    //     {
    //         // as single card
    //         let g = new Group();
    //         g.cards.push( curValue );
    //         result.addChild(g);
    ...
    //     // as bai da jiang or ke que
    //     if ( vBaiDa != null && vBaiDa.length >= 1 &&  result.isHaveSingleCard() == false && ( result.getPairCnt() == 0 || ( false == ischeckHu && result.getPairCnt() == 1 && result.isHaveNotShun() == false ) ))
    //     {
    //         let bv = vBaiDa[0] ;
    //         let gb = new Group();
    //         gb.cards.push( curValue,bv );
    //         result.addChild(gb);
    //         vBaiDa.splice(0,1);

    //         let vSubb : number[] = [] ;
    //         vSubb = vSubb.concat(cards);
    //         let ok = this.checkCards(gb,vSubb,mustKeZi,vBaiDa,isQuick,ischeckHu ) ;
    //         if ( ok == false )
    //         {
    //             result.popChild();
    //         }
