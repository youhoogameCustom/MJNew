import IDlgRecorderDataHY, { IRecorderItemDataHY, RecorderOffset } from "../dlgRecorder/IDlgRecorderDataHY";
...
export default class ClubDataRecorderHY extends ClubDataRecorder implements IDlgRecorderDataHY {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && (xhr.status >= 200 && xhr.status < 400)) {
                ...
                else
                {
                    ...
                    self.doInformDataRefreshed(true);
                }
            }
        };
        xhr.open("GET", url, true);
    ...
    {
        super.doInformDataRefreshed(isResetInterval);
        if ( null != this.pCallBackRecorderRefresh )
        {
            this.pCallBackRecorderRefresh();
        }
    }
